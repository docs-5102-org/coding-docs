...
Java虚拟机在运行时将内存划分为不同的数据区域。JDK 1.8前后的主要区别是**方法区的实现方式**：

#### JDK 1.8之前：使用永久代（PermGen）

```

运行时数据区域 (Runtime Data Areas) - JDK 1.7
+-----------------------------------------------------------------------------+
|                                                                             |
|  +------------------------------ 线程共享 -------------------------------+  |
|  | (Thread Shared)                                                       |  |
...
|  |  |  +-----------------+  |          |  +------------------------+  |  |  |
|  |  |  | 字符串常量池    |  |           |  | 运行时常量池            |  |  |  |
|  |  |  | (String Pool)   |  |          |  | (Runtime Const Pool)   |  |  |  |
|  |  |  +-----------------+  |          |  +------------------------+  |  |  |
|  |  +-----------------------+          +------------------------------+  |  |
|  |                                                                       |  |
|  +-----------------------------------------------------------------------+  |
...
#### JDK 1.8及以后：使用元空间（Metaspace）

```

                          RUNTIME DATA AREA (JDK 1.8)
+-------------------------------------------------------------------------------+
|                                                                    [Shared]   |
|   +-----------------------------------------------------------------------+   |
|   |                                 HEAP                                  |   |
...
|      |  +-------------------+  |         |  +-------------------+  |          |
|      |  +-------------------+  |         |  +-------------------+  |          |
|      |  | Native Method Stk |  |         |  | Native Method Stk |  |          |
|      |  +-------------------+  |         |  +-------------------+  |          |
|      |  +-------------------+  |         |  +-------------------+  |          |
...
- 字面量：字符串、数值常量
- 符号引用：类引用、字段引用、方法引用
- 14 种常量类型（CONSTANT_Utf8、CONSTANT_Class、CONSTANT_Methodref 等）
