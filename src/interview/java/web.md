...
**（Prepared Statement）为什么能防止 SQL 注入的好问题。**

核心原因在于：**SQL 解析和数据输入完全分离成了两个阶段**。

<img :src="$withBase('/assets/images/interview/sql_safe.png')" 
  alt=""
  width="800px" 
  height="auto">
...
SQL 的结构（哪里是关键字、哪里是条件、哪里是值）在编译阶段就已经确定了。`?` 只是两个「值的槽位」，后续传入的任何内容都只会被数据库当成字符串字面量处理，根本没有机会参与语法解析。

**分三步理解：**

**第一步：发送模板** — `conn.prepareStatement(sql)` 把带 `?` 的模板发给数据库，数据库编译它、理解它的结构，此时还没有任何用户数据介入。

**第二步：锁死结构** — 编译完成后，这条查询的语法树已经固定。`?` 的角色是「一个字符串值」，这个身份不会再变了。

**第三步：填入数据** — `pstmt.setString(1, username)` 把用户输入塞进去。哪怕用户输入的是 `' OR '1'='1`，数据库也只会去找用户名字面上等于这串字符的记录，单引号不再具有任何语法意义。
...
// 如果 username = "' OR '1'='1"
// 最终 SQL 变成：WHERE username='' OR '1'='1'
// '1'='1' 永远为真 → 绕过登录
```

这就是注入的根本原因：**数据混入了代码**。Prepared Statement 通过在架构层面隔离「代码」和「数据」的处理时机，从根本上消除了这个漏洞，而不是靠过滤某些特殊字符来打补丁。
:::
