...
<iframe
  src="https://tuonioooo.github.io/fast-web-template/web/gateway/gateway-architecture-visualizer/dist/index.html"
  width="100%"
  height="800"
  style="border:0;"
></iframe>
...
| AddRequestParameter | 为原始请求添加请求参数 | 参数名称及值 | - AddRequestParameter=my-request-param, passjava.cn |
| AddResponseHeader | 为原始响应添加Header | Header的名称及值 | - AddResponseHeader=my-response-header,passjava.cn |
| DedupeResponseHeader | 剔除响应头中重复的值 | 需要去重的Header名称及去重策略 | |
| Hystrix | 为路由引入Hystrix的断路器保护 | HystrixCommand的名称 | |
| FallbackHeaders | 为fallbackUri的请求头中添加具体的异常信息 | Header的名称 | |
| PrefixPath | 为原始请求路径添加前缀 | 前缀路径 | - PrefixPath=/consumer |
| PreserveHostHeader | 为请求添加一个preserveHostHeader=true的属性,路由过滤器会检查该属性以决定是否要发送原始的Host | 无 | - PreserveHostHeader |
| RequestRateLimiter | 用于对请求限流,限流算法为令牌桶 | keyResolver、rateLimiter、statusCode、denyEmptyKey、emptyKeyStatus | |
| RedirectTo | 将原始请求重定向到指定的URL | http状态码及重定向的url | - RedirectTo=302, http://www.passjava.cn |
| RemoveHopByHopHeadersFilter | 为原始请求删除IETF组织规定的一系列Header | 默认就会启用,可以删除这些默认指定区删除哪些Header | |
| RemoveRequestHeader | 为原始请求删除某个Header | Header名称 | - RemoveRequestHeader=my-request-header |
| RemoveResponseHeader | 为原始响应删除某个Header | Header名称 | - RemoveResponseHeader=my-response-header |
| RewritePath | 重写原始的请求路径 | 原始路径正则表达式以及重写后路径的正则表达式 | - RewritePath=/api/(?<segment>.*), /$\{segment} |
| RewriteResponseHeader | 重写原始响应中的某个Header | Header名称,值的正则表达式,重写后的值 | - RewriteResponseHeader=X-Response-Foo, password=[^&]+, password=*** |
| SaveSession | 在转发请求之前,强制执行WebSession::save操作 | 无 | - SaveSession |
...
| RequestSize | 设置允许接收最大请求包的大小,如果请求包大小超过设置的值,则返回 413 Payload Too Large | 请求包大小,单位为字节,默认值为5M | - name: RequestSize args: maxSize: 5000000 |
...
| 全局过滤器 | 作用 |
|---|---|
| ForwardRoutingFilter | 用于本地 forward,也就是将请求在 Gateway 服务内进行转发,而不是转发到下游服务 |
| LoadBalancerClientFilter | 整合 Ribbon 实现负载均衡 |
| NettyRoutingFilter | 使用 Netty 的 HttpClient 转发 http、https 请求 |
| NettyWriteResponseFilter | 将代理响应写回网关的客户端侧 |
| RouteToRequestUrlFilter | 将从 request 里获取的原始 url 转换成 Gateway 进行请求转发时所使用的 url |
| WebsocketRoutingFilter | 使用 Spring Web Socket 将转发 Websocket 请求 |
